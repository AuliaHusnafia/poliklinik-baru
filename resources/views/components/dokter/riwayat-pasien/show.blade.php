<x-layouts.app title="Detail Riwayat Pasien">
    ...
    <div class="w-full flex flex-col gap-5">
        ...
        {{-- Hitung total biaya --}}
@php
    $totalHargaObat = $periksa->detailPeriksas->sum(fn($d) => $d->obat->harga ?? 0);
    $totalBiaya = 150000 + $totalHargaObat;
@endphp

{{-- Total Biaya --}}
<div class="mt-4 p-3 bg-gray-50 rounded border border-gray-200">
    <span class="font-semibold">Total Biaya Periksa:</span>
    <span class="font-bold ml-2">Rp {{ number_format($totalBiaya, 0, ',', '.') }}</span>
</div>
